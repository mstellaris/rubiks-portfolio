...
import { DimensionalShift } from './effects/DimensionalShift.js';
...
const dimensionalShift = new DimensionalShift(cube, scene, camera, particles);

// Override navigateTo: trigger dimensional shift instead of overlay
faceLink.navigateTo = function(section) {
  const face = [...faceLink.activeLinks.entries()]
    .find(([, link]) => link.section === section)?.[0];
  if (face) {
    dimensionalShift.enter(face, section);
  }
};
...
scrambleBtn?.addEventListener('click', () => {
  if (dimensionalShift.isActive) dimensionalShift.reverse();
  faceLink.hideAll();
  cube.scramble(25);
});
