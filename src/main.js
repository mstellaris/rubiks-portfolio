...
import { HologramProjection } from './effects/HologramProjection.js';
...
const hologram = new HologramProjection(scene, camera, cube);

// Override navigateTo: project hologram instead of overlay
faceLink.navigateTo = function(section) {
  const face = [...faceLink.activeLinks.entries()]
    .find(([, link]) => link.section === section)?.[0];
  if (face) hologram.project(face, section);
};
