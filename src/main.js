...
import { FaceUnfold } from './effects/FaceUnfold.js';
...
// Face unfold system
const faceUnfold = new FaceUnfold(cube, scene, camera);

// Override navigateTo: unfold face instead of overlay
faceLink.navigateTo = function(section) {
  const face = [...faceLink.activeLinks.entries()]
    .find(([, link]) => link.section === section)?.[0];
  if (face) faceUnfold.unfold(face, section);
};
